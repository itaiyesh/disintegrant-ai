...
public class CharacterAttributes : MonoBehaviour
{
    ...
    public float UnderAttackDuration = 1.5f;
    ...
    void Update()
    {
        foreach (CharacterModifier modifier in Modifiers) // Iterate over attributes
        {
            ...
            else if (modifier.DurationRemaining <= 0)
            { // Attribute duration finished, remove attribute
        ...
        if (Time.fixedTime - lastAttacked > UnderAttackDuration)
        {
            this.characterAttributes.IsUnderAttack = false;
        }
    }

    public void Attacked(GameObject projectile)
    {
        this.characterAttributes.IsUnderAttack = true;
        lastAttacked = Time.fixedTime;
    }
